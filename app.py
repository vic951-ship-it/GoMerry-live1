...
ADMIN_KEY="merry2025"
PAYPAL_ID="PAYPAL_CLIENT_ID_PLACEHOLDER"  # will be replaced by picker itself, put your real ID in picker.html for now
...
def init_db():
 con=db()
 con.executescript("""
 CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,phone TEXT UNIQUE,range_min INTEGER,range_max INTEGER,amount INTEGER,lifetime_amount INTEGER,lifetime_paid INTEGER DEFAULT 0,paypal_order_id TEXT,created_at TEXT);
 CREATE TABLE IF NOT EXISTS groups(id INTEGER PRIMARY KEY AUTOINCREMENT,range_min INTEGER,range_max INTEGER,group_number INTEGER,status TEXT DEFAULT 'OPEN',total_amount INTEGER DEFAULT 0,beneficiary_id INTEGER,created_at TEXT);
 CREATE TABLE IF NOT EXISTS group_members(id INTEGER PRIMARY KEY AUTOINCREMENT,group_id INTEGER,user_id INTEGER,amount INTEGER,UNIQUE(group_id,user_id));
 CREATE TABLE IF NOT EXISTS payments(id INTEGER PRIMARY KEY AUTOINCREMENT,phone TEXT,order_id TEXT,type TEXT,amount REAL,created_at TEXT);
 """)
 con.commit()
 con.close()
init_db()
...
@app.route("/")
def home():
 return send_file(os.path.join(BASE,"picker.html"))

@app.route("/api/verify_registration",methods=["POST"])
def verify_registration():
 d=request.get_json()
 con=db()
 con.execute("INSERT INTO payments(phone,order_id,type,amount,created_at) VALUES(?,?,?,?,?)",(d.get("phone"),d.get("orderID"),"REGISTRATION",1,datetime.now().isoformat()))
 u=con.execute("SELECT * FROM users WHERE phone=?",(d.get("phone"),)).fetchone()
 if not u:
  con.execute("INSERT INTO users(name,phone,lifetime_paid,paypal_order_id,created_at) VALUES(?,?,1,?,?)",(d.get("name",""),d.get("phone"),d.get("orderID"),datetime.now().isoformat()))
 else:
  con.execute("UPDATE users SET lifetime_paid=1,paypal_order_id=? WHERE phone=?",(d.get("orderID"),d.get("phone")))
 con.commit()
 con.close()
...
def join():
 ...
 rmin=int(d.get("range_min")); rmax=int(d.get("range_max")); amt=int(d.get("amount"))
 con=db(); cur=con.cursor()
 u=con.execute("SELECT * FROM users WHERE phone=?",(phone,)).fetchone()
 if not u or u["lifetime_paid"]==0:
  con.close()
  return jsonify({"error":"Pay $1 registration first!"}),400
 ...
 uid=ex["id"]
 g=cur.execute("SELECT g.id FROM groups g JOIN group_members gm ON gm.group_id=g.id WHERE gm.user_id=? AND g.range_min=? AND g.range_max=?",(uid,rmin,rmax)).fetchone()
 if g:
  con.close()
