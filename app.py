...
def join():
 ...
 if ex and ex["lifetime_paid"]==1:
  con.close()
  return jsonify({"error":"Lifetime already paid"}),400
 if not ex:
  cur.execute("INSERT INTO users(name,phone,range_min,range_max,amount,lifetime_amount,created_at) VALUES(?,?,?,?,?,?,?)",(name,phone,rmin,rmax,amt,life,datetime.now().isoformat()))
  uid=cur.lastrowid
 else:
  uid=ex["id"]
...
@app.route("/api/pay_lifetime",methods=["POST"])
def pay_lifetime():
 phone=request.get_json().get("phone")
 con=db()
 con.execute("UPDATE users SET lifetime_paid=1 WHERE phone=?",(phone,))
 con.commit()
 con.close()
